...
```bash
go get -tool github.com/kda47/envdoc@latest
go install tool
```
...
```go
//go:generate go run github.com/kda47/envdoc@latest -output environments.md
type Config struct {
    // ...
}
```
...
```bash
go install github.com/kda47/envdoc@latest
```
